...
function PoliceFinesCarInfoForm() {
  ...
  const columns = [
    ...
    { field: "documentNo", headerName: "დოკუმენტის ნომერი", width: 180 },
    {
      field: "actions",
      headerName: "Actions",
      width: 250,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${REACT_APP_API_BASE_URL}/PoliceFinesCarInfo/create`,
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `${REACT_APP_API_BASE_URL}/PoliceFinesCarInfo/delete/${id}`
      );
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await axios.put(
        await axios.delete(
          `${REACT_APP_API_BASE_URL}/PoliceFinesCarInfo/update/${selectedRow.id}`
        ),
  ...
  const handleExportToExcel = () => {
    ...
    const filteredData = data.map(({ vehicleNo, documentNo }) => ({
      vehicleNo,
      documentNo,
    }));
    ...
    XLSX.writeFile(wb, `პოლიცია_ავტომობილების_ინფორმაცია_${currentDate}.xlsx`);
  };
      ...
      const newEntries = data.filter((entry) => {
        ...
        return !existingData.some(
          (existing) =>
            ...
            existing.documentNo === entry.documentNo
        );
      });
...
export default PoliceFinesCarInfoForm;
